package com.example.demowebmodule4.service.impl;

import com.example.demowebmodule4.model.Product;
import com.example.demowebmodule4.repository.InterfaceProductRepo;
import com.example.demowebmodule4.service.InterfaceCrudService;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;
import org.springframework.stereotype.Service;

import java.util.List;

@Service
public class ProductServiceImpl implements InterfaceCrudService<Product> {
    @Autowired
    InterfaceProductRepo interfaceProductRepo;

    @Override
    public List<Product> showAll() {
        return (List<Product>) interfaceProductRepo.findAll();
    }

    @Override
    public void create(Product product) {
        interfaceProductRepo.save(product);
    }

    @Override
    public void update(Product product) {
        interfaceProductRepo.save(product);
    }

    @Override
    public void delete(Product product) {
        interfaceProductRepo.delete(product);
    }
    public Product findProduct(int id){
        return interfaceProductRepo.findById(id).get();
    }
    public Page<Product> showAll(Pageable pageable){
        return interfaceProductRepo.findAll(pageable);
    }
}
